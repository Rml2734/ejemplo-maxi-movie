...
using Microsoft.Extensions.AI; // Este using habilita el método CompleteAsync
using System;
...
namespace maxi_movie_mvc.Service
{
    public class LlmService
    {
        ...
        private async Task<string> ConsultarLlmAsync(string prompt)
        {
            try
            {
                // Usar GetResponseAsync según la interfaz IChatClient
                var messages = new[] { new ChatMessage(ChatRole.User, prompt) };
                var response = await _chatClient.GetResponseAsync(messages);

                // Obtener el texto de la última respuesta del asistente
                var lastAssistantMessage = response.Messages
                    .LastOrDefault(m => m.Role == ChatRole.Assistant);

                return lastAssistantMessage?.Text?.Trim() ?? "No se pudo obtener una respuesta.";
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException($"Error al procesar la solicitud con Gemini: {ex.Message}", ex);
            }
        }
